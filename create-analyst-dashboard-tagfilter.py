...
weekly-growth chart:
    ...
    BEFORE (create-analyst-dashboard.py):
        "Analyst · New Features per Week (by tag value)"
            -> echarts bar grouped by `tag_value`, i.e. EVERY tag value is
               exploded into its own stacked series.

    AFTER (this file):
        "Analyst (Tag Filter) · New Features per Week"
            -> echarts bar of a single "features" series per week (no explosion).
               You pick which tag value(s) to slice by from the "Tag value"
               native-filter DROP-DOWN selection box, which is scoped to drive
               this chart. With nothing selected the chart shows all features;
               select one or more tag values and the weekly bars filter to just
               those features.

Everything else (datasets, the usage/reuse charts, the status chart, the other
native filters) is identical to the original analyst dashboard.

To keep the two dashboards independent, this builder uses its own dashboard
TITLE and its own chart-name PREFIX, so re-running it never disturbs the
original "Analyst/Data Scientist Dashboard".

Run:  python create-analyst-dashboard-tagfilter.py
"""
import argparse
import json
...
PREFIX = "Analyst (Tag Filter) · "              # namespaces charts for idempotency
...
def build_json_metadata(ds_id, excluded):
    return json.dumps({
        "native_filter_configuration": [
            ...
            # Multi-select DROP-DOWN: this is the selection box that drives the
            # "New Features per Week" chart. Pick one or more tag values to
            # filter the weekly bars; leave empty to show all features.
...
def main():
    ...
    if not tags:
        print("No tags found — the 'Tag value' drop-down will only offer '(untagged)'.")
    ...
    # CHANGED vs create-analyst-dashboard.py: no `groupby` so tag values are NOT
    # exploded into stacked series. A single "features" bar per week; slice it
    # with the "Tag value" native-filter drop-down (scoped to this chart).
    add("New Features per Week", "echarts_timeseries_bar", ds_inv,
        weekly_bar(FEATURES_METRIC, []), 12, 52, inventory=True)
